...
class TraHoliday_Ctrl
{
  ...
  */
  function check($PI_ary){
    ...
    // 開始時
    $PO_ary = check_nomalItem('開始時',$PI_ary, 2, $PO_ary);
    if(chk_while($PI_ary['開始時'], 0, 23) == 結果['問題あり']){
      $PO_ary['status'] = 結果['問題あり'];
      $PO_ary['msg']['開始時'] = エラーメッセージ['chk_while'];
    }
    // 時の方で終了時間未満かチェックする
    if(chk_over($PI_ary['開始時'].$PI_ary['開始分'], $PI_ary['終了時'].$PI_ary['終了分']) == 結果['問題あり']){
      $PO_ary['status'] = 結果['問題あり'];
      $PO_ary['msg']['開始時'] = エラーメッセージ['chk_over'];
    }

    // 開始分
    $PO_ary = check_nomalItem('開始分',$PI_ary, 2, $PO_ary);
    if(chk_while($PI_ary['開始分'], 0, 59) == 結果['問題あり']){
      $PO_ary['status'] = 結果['問題あり'];
      $PO_ary['msg']['開始分'] = エラーメッセージ['chk_while'];
    }

    // 終了時
    $PO_ary = check_nomalItem('終了時',$PI_ary, 2, $PO_ary);
    if(chk_while($PI_ary['終了時'], 0, 29) == 結果['問題あり']){
      $PO_ary['status'] = 結果['問題あり'];
      $PO_ary['msg']['終了時'] = エラーメッセージ['chk_while'];
    }
    // 終了分
    $PO_ary = check_nomalItem('終了分',$PI_ary, 2, $PO_ary);
    if(chk_while($PI_ary['終了分'], 0, 59) == 結果['問題あり']){
      $PO_ary['status'] = 結果['問題あり'];
