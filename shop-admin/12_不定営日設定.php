...
<?php 
  ...
  function update(){
    ...
    /* エラーチェック */
      $res_check = $cls_Tra不定営->check($更新内容);
    ...
    /* トランザクション*/
      try{
        ...
        $return_dateUpdIns = data_Update_or_Insert($cls_Tra不定営, $更新内容);
    ...
    /* トランザクション*/
      try{
        ...
        for ($seq = 1; $seq <= $_POST['reg_MaxSeq']; $seq++){
          ...
          // エラー無しで更新処理実行
          $sql_del = $cls_Tra不定営->delete($削除内容);
...
<?php
  ...
  $cls_Tra不定営 = new TraSale_Ctrl;
  $Tra不定営_更新情報 = array(); // sqlを作るための引数用
  ...
  /* 最後に必ずマスタ最新値を取得し直す */
    $cls_Tra不定営 = new TraSale_Ctrl;
    $Tra不定営_検索条件 = array(); // sqlを作るための引数用
    $ary_不定営 = array(); // ログインショップIDのレコード
    ...
    // ショップIDを条件に検索
    $Sql_不定営fromNow = $cls_Tra不定営->select_fromNow($Tra不定営_検索条件);
    $res_select = $cls_dbCtrl->select($Sql_不定営fromNow);
    if ($res_select["status"] == false){
      $エラーコード = ERR種類['マスタ情報取得失敗'];
    }else{
      // 取得レコードを変数へ
      $ary_不定営 = $res_select["rows"];
    }
  /*  */
...
<script language="javascript" type="text/javascript">
  ...
  function kindChange(PI_val) {
    ...
    if(PI_val == 0){
      ...
      document.getElementById( "開始時間タイトルlbl").className = 'noedit-lbl';
      document.getElementById( "終了時間タイトルlbl").className = 'noedit-lbl';
...
<div class="content-item">
  ...
  <form class="delete" action="" method="post">
    <input type="hidden" name="menu-btn" value="<?php echo $index['Me']['NO']; ?>" />
    <input type="hidden" name="reg_MaxSeq" value="<?php echo count($ary_不定営); ?>" />
    <?php
      if (count($ary_不定営) == 0){
        //処理日以降の不定営設定はない
        echo '<h4>有効な不定営設定はありません。</h4>';
      }
    ?>
    <table id="reg-table">
      <tr>
        ...
        <?php } ?>
        <th class="clm-info">不定営内容</th>
        <th class="clm-delBtn">削除</th>
      </tr>
      
      <?php for($seq = 1; $seq <= count($ary_不定営); $seq++){
          $row = $ary_不定営[$seq - 1];
          // 曜日を取得
